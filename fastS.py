...
if __name__ == "__main__":
    ...
    #url = input("Search here: ")
    url=(sys.argv[1])
    query = url
    for j in search(query, tld="co.in", num=1, stop=1, pause=2): 
        #print(j) 
        respond = requests.get(j)
        link2=j
    ...
    sum2= [x.strip() for x in summary]
    
    for n, i in enumerate(sum2):
        if i == '':
            sum2[n] = ' '
    listToStr = ''.join([str(elem) for elem in sum2])
...
sc.render_graph(domains)
    #bot.send_message(chat_id=chat_id, text=link2 )
